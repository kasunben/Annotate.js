...
- **Access control** — ownership-based, not auth-based; `_authorId` UUID generated by `generateId()` on first load, stored in `localStorage` as `annotate_author_id`; attached to every Thread and Reply as `authorId`; `_isOwner(item)` returns `false` for items with `authorId: null` (legacy, pre-access-control → permanently read-only); Edit/Delete UI buttons only rendered/wired for own threads/replies (`_isOwner` check in `_renderSavedCard` and `_buildReplyEl`); Resolve is unwrapped — anyone can resolve (collaborative action); server enforces ownership on `POST /threads` upsert via `checkOwnership()`; `DELETE /threads?authorId=` scopes deletion to own threads; `dbClearMyThreads()` does an IDB full-scan delete filtered by `siteId + authorId`; **known limitations**: localStorage clear = permanent loss of edit access; UUID copy = impersonation possible (deliberate effort required); P2P has no server enforcement; legacy threads (no `authorId`) are permanently read-only server-side (operator can reclaim: `UPDATE threads SET author_id = '…' WHERE author_id IS NULL`)
