...
├── docker-compose.yml       # Named volume for SQLite persistence; PORT env var supported
...
Two supported paths — both start with `npm install && npm run build`:

**Docker** (PaaS / containerized VPS):
```bash
docker compose up -d   # SQLite persisted in named volume annotate_data
