...
- ✅ **Sidebar scroll — non-Threads panels stay in viewport** — `.annotate-panel` uses `position: sticky; top: var(--annotate-header-h); height: calc(100vh - var(--annotate-header-h))` so Resolved / Activity / Settings content stays visible as the page scrolls; Threads panel retains `position: absolute` (cards align with their highlight positions); `--annotate-header-h` is measured once at boot via `offsetHeight` on the sidebar header element
...
- **Sidebar scroll — non-Threads panels sticky** — `.annotate-panel` is `position: sticky; top: var(--annotate-header-h); height: calc(100vh - var(--annotate-header-h)); overflow-y: auto` so Resolved / Activity / Settings content stays in the viewport as the page scrolls; the Threads panel and the overall `#annotate-sidebar` remain `position: absolute` so card `_anchorTop` alignment with highlights is unaffected; `--annotate-header-h` is set once at boot by measuring `document.getElementById('annotate-sidebar-header').offsetHeight` and calling `sidebar.style.setProperty`
