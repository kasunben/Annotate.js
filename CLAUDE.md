...
- **Resolved threads are frozen** — when `thread.resolved` is true, `_renderSavedCard` hides the Delete menu item for **everyone** (including the owner) and hides the Reply trigger; the action-row Resolve button is rebuilt as **Un-Resolve**, open to anyone like Resolve itself; Edit is retained for the owner so they can still correct text; the click handler on `.annotate-resolve-btn` toggles `t.resolved` and emits `thread_unresolved` activity when reverting; `_renderResolvedTab` no longer hides the action row — the per-card logic in `_renderSavedCard` produces the correct frozen UI; reply-trigger wiring uses a null-guard because the button isn't rendered on resolved cards
