...
Annotate.js/
├── assets/
│   └── js/annotate.js       # Library source (single IIFE — DB + anchor + sync + UI all inlined)
├── annotate.min.js          # Production build — gitignored, generated by `npm run build`
├── demo/
...
├── Dockerfile               # Multi-stage: terser build in stage 1, lean Node runtime in stage 2
...
├── .nvmrc                   # Pins Node 23
├── package.json             # express + cors (prod); terser (dev); engines: node>=23
├── README.md
└── docs/
...
- ✅ `npm run build` — terser minifies `assets/js/annotate.js` → `annotate.min.js` (project root, served at `/annotate.min.js`)
